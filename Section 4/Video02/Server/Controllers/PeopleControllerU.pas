...
uses
  MVCFramework, PeopleModuleU, PersonBO, Vcl.Imaging.pngimage;
...
uses
  SysUtils, ObjectsMappers, MVCFramework.Commons, System.JSON,
...
begin
  case CTX.Request.HTTPMethod of
    httpPOST:
      begin
        LFilters := CTX.Request.BodyAsJSONObject;
        if not Assigned(LFilters) then
          raise Exception.Create('Invalid search parameters');
        LSearchText := Mapper.GetStringDef(LFilters, 'TEXT');
      end;
    httpGET:
      begin
