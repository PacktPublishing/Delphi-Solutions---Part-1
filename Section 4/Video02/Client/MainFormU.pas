...
uses
  ...
  Data.DB, Vcl.Grids, Vcl.DBGrids, Datasnap.DBClient, REST.Response.Adapter, Vcl.StdCtrls, Vcl.Mask,
...
uses
  ObjectsMappers, MVCFramework.Commons, System.NetEncoding;
...
begin
  ...
  begin
    ...
    begin
      ShowMessageFmt('Invalid response format (expected application/json, actual %s',
        [LResponse.HeaderValue['Content-Type']]);
    end;
  end
  else
  begin
    ShowMessageFmt(ERROR_FORMAT_STRING, [LResponse.StatusCode, LResponse.StatusText]);
  end;
end;
...
begin
  ...
  if LResponse.StatusCode <> HTTP_STATUS.Created then
    raise Exception.CreateFmt(ERROR_FORMAT_STRING, [LResponse.StatusCode, LResponse.StatusText]);
...
var
  LResponse: IHTTPResponse;
begin
  LResponse := FHTTPClient.Delete(BASEURL + '/people/' + ADataSet.FieldByName('ID').AsString);
...
var
  ...
  LBody: TStringStream;
begin
  LPUTRequest := FHTTPClient.GetRequest('PUT', BASEURL + '/people/' + ADataSet.FieldByName('ID')
    .AsString);
